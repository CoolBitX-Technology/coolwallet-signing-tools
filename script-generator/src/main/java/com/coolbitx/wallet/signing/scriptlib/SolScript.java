...
public class SolScript {
    ...
    public static void listAll() {
        ...
        System.out
            .println("Sol create and transfer spl token-2022: \n" + getCreateAndTransferSplToken22Script() + "\n");
    ...
    public enum TxType {
        TRANSFER("01"), TRANSFER_WITH_COMPUTE_BUDGET("03"), CREATE_AND_TRANSFER("02"),
    ...
    public enum StakeType {
        DELEGATE, DELEGATE_AND_CREATE, UNDELEGATE, WITHDRAW;
    }
    ...
    public static String getTransferScriptSignature = Strings.padStart(
        "304502201feef74de887b6d8513f56371cebf5e9d7f05e2ddbef25eb6dd91cd088910018022100d4afd1ea9d5f9ae142cdab5419e8c0974f10651376af03a141a26e49b08691dc",
        144, '0');
    ...
    public static String getTransferWithComputeBudgetScriptSignature = Strings.padStart(
        "3044022032851d9ca464de5ae92c063f33ada384fa0dee27d6e3d469197a367ef9cc9cbe0220137ffa77407186912ffbb5fd74529b3818c7e8fa1d4eb13d4a924a44cb7caaf9",
        144, '0');
    ...
    private static String transferScript(TxType txType) {
        ...
        ScriptAssembler script = new ScriptAssembler().setCoinType(0x01f5).copyArgument(header).copyArgument(keysCount)
            .copyArgument(publicKey0).copyArgument(publicKey1)
        ...
        if (txType == TxType.TRANSFER_WITH_COMPUTE_BUDGET) {
            script
                ...
                .copyArgument(gasPriceProgramIdIndex).copyArgument(gasPriceKeyLength).copyArgument(gasPriceDataLength)
                .copyArgument(gasPriceComputeBudgetInstructionType).copyArgument(gasPrice)
                // Compute Budget: Set Compute Unit Limit instruction
                .copyArgument(gasLimitProgramIdIndex).copyArgument(gasLimitKeyLength).copyArgument(gasLimitDataLength)
                .copyArgument(gasLimitComputeBudgetInstructionType) // SetComputeUnitLimit
                .copyArgument(gasLimit);
        }
        script
            // Transfer instruction
            .copyArgument(systemProgramIdIndex).copyArgument(keyIndicesLength).copyArgument(fromKeyIndex)
            .copyArgument(toKeyIndex).copyArgument(dataLength).copyArgument(instruction).copyArgument(amount)
            .showMessage("SOL")
            .ifEqual(toKeyIndex, "00", new ScriptAssembler()
                .baseConvert(publicKey0, Buffer.CACHE2, 0, ScriptAssembler.base58Charset, ScriptAssembler.zeroInherit)
                .showAddress(ScriptData.getDataBufferAll(Buffer.CACHE2)).getScript(), "")
            .ifEqual(toKeyIndex, "01", new ScriptAssembler()
                .baseConvert(publicKey1, Buffer.CACHE2, 0, ScriptAssembler.base58Charset, ScriptAssembler.zeroInherit)
                .showAddress(ScriptData.getDataBufferAll(Buffer.CACHE2)).getScript(), "")
            .clearBuffer(Buffer.CACHE2)
            .baseConvert(amount, Buffer.CACHE1, 8, ScriptAssembler.binaryCharset, ScriptAssembler.inLittleEndian)
            .showAmount(ScriptData.getDataBufferAll(Buffer.CACHE1), 9).clearBuffer(Buffer.CACHE1).showPressButton()
            .setHeader(HashType.NONE, SignType.EDDSA).getScript();
    ...
    public static String getSolSmartScript() {
        ...
        return scriptAsb.setCoinType(0x01f5).copyArgument(data).showMessage("SOL").showWrap("SMART", "")
            .showPressButton().setHeader(HashType.NONE, SignType.EDDSA).getScript();
    }
    ...
    public static String getSignInScript() {
        ...
        ScriptAssembler scriptAsb = new ScriptAssembler();
        return scriptAsb.setCoinType(0x01f5).setBufferIntUnsafe(domainLength).copyArgument(domain).copyString(fixedText)
            .derivePublicKey(argAddressPath, Buffer.CACHE2)
            .baseConvert(ScriptData.getDataBufferAll(Buffer.CACHE2), Buffer.TRANSACTION, 0,
                ScriptAssembler.base58Charset, ScriptAssembler.zeroInherit)
            .setBufferIntUnsafe(optionalMessageLength).copyArgument(optionalMessage).showMessage("SOL")
            .showWrap("SIGN IN", "").showPressButton().setHeader(HashType.NONE, SignType.EDDSA).getScript();
    }

    public static String getSignInScriptSignature = Strings.padStart(
        "3045022100ce2624f80ac5234d77415ca9df9ad153fa1c8baea28497d937e619ce63dddb6302205bb1f1d8aa5a47e5c87c615be03cf0a375c40ed3e19746e3279fd74dbddca82a",
        144, '0');
    ...
    public static String getSignMessageScript() {
        ...
        ScriptAssembler scriptAsb = new ScriptAssembler();
        return scriptAsb.setCoinType(0x01f5).copyArgument(data).showMessage("SOL").showWrap("MESSAGE", "")
            .showPressButton().setHeader(HashType.NONE, SignType.EDDSA).getScript();
    }

    public static String getSignMessageScriptSignature = Strings.padStart(
        "30450220482a5109dd908b886d4b8cd8b4bfb43a56c65232d68e4b30798d3cdbb55e31f3022100ee6e9c3d9468a8ad29ca6ac2b08719c7cbe7e857951a1b7ffa62e3445bc1bb10",
        144, '0');
    ...
    private static String splTransferScript(TxType txType) {
        ...
        if (txType == TxType.CREATE_AND_TRANSFER || txType == TxType.CREATE_AND_TRANSFER_WITH_COMPUTE_BUDGET) {
            script.copyArgument(publicKey0).copyArgument(publicKey1).copyArgument(publicKey2).copyArgument(publicKey3)
                .copyArgument(publicKey4).copyArgument(publicKey5).copyArgument(publicKey6).copyArgument(publicKey7)
        ...
        if (txType == TxType.CREATE_AND_TRANSFER || txType == TxType.CREATE_AND_TRANSFER_WITH_COMPUTE_BUDGET) {
            script
                ...
                .copyArgument(createAssociatedProgramIdIndex).copyArgument(createAssociatedKeyIndicesLength)
                .copyArgument(createAssociatedKeyIndices).copyArgument(createAssociatedDataLength);
        }
        if (txType == TxType.TRANSFER_WITH_COMPUTE_BUDGET || txType == TxType.CREATE_AND_TRANSFER_WITH_COMPUTE_BUDGET) {
            script
                // Compute Budget: Set Compute Unit Price instruction
        ...
        script
            ...
            .copyArgument(tokenProgramIdIndex).copyArgument(keyIndicesLength).copyArgument(keyIndices)
            .copyArgument(dataLength).copyArgument(tokenInstruction).copyArgument(tokenAmount)
            .copyArgument(customTokenDecimals);

        script.showMessage("SOL")
            ...
            .showAmount(ScriptData.getDataBufferAll(Buffer.CACHE1), ScriptData.bufInt).clearBuffer(Buffer.CACHE1)
            .showPressButton().setHeader(HashType.NONE, SignType.EDDSA);
        return script.getScript();
    }
    ...
    public static String getDelegateAndCreateAccountWithSeedScript() {
        ...
        return scriptAsb.setCoinType(0x01f5).copyArgument(header).copyArgument(keysCount).copyArgument(publicKey0)
            .copyArgument(publicKey1).copyArgument(publicKey2).copyArgument(publicKey3).copyArgument(publicKey4)
            .copyArgument(publicKey5).copyArgument(publicKey6).copyArgument(publicKey7).copyArgument(publicKey8)
            .ifEqual(publicKey9, EMPTY_PUBLIC_KEY, "", new ScriptAssembler().copyArgument(publicKey9).getScript())
            .copyArgument(recentBlockhash).copyArgument(instructionsCount)
            .ifEqual(gasPriceDataLength, "00", "",
                new ScriptAssembler().copyArgument(gasPriceProgramIdIndex).copyArgument(gasPriceKeyLength)
                    .copyArgument(gasPriceDataLength).copyArgument(gasPriceComputeBudgetInstructionType)
                    .copyArgument(gasPrice).getScript())
            .ifEqual(gasLimitDataLength, "00", "",
                new ScriptAssembler().copyArgument(gasLimitProgramIdIndex).copyArgument(gasLimitKeyLength)
                    .copyArgument(gasLimitDataLength).copyArgument(gasLimitComputeBudgetInstructionType)
                    .copyArgument(gasLimit).getScript())
            .copyArgument(createAccountProgramIdIndex).copyArgument(createAccountKeyIndicesLength)
            .copyArgument(createAccountKeyIndices).copyArgument(createAccountDataLength)
            .copyArgument(createAccountInstruction) // 03000000
            .copyArgument(createAccountBase).copyArgument(createAccountSeedLength)
            .copyArgument(createAccountSeedPaddingLength).copyArgument(createAccountSeed)
            .copyArgument(createAccountLamports).copyArgument(createAccountSpace).copyArgument(createAccountProgramId)
            .copyArgument(initializeProgramIdIndex).copyArgument(initializeKeyIndicesLength)
            .copyArgument(initializeKeyIndices).copyArgument(initializeDataLength).copyArgument(initializeData)
            .copyArgument(delegateProgramIdIndex).copyArgument(delegateKeyIndicesLength).copyArgument(delegateKeyIndex0)
            .copyArgument(delegateKeyIndex1).copyArgument(delegateKeyIndexOthers).copyArgument(delegateDataLength)
            .copyArgument(delegateData) // 02000000
            .showMessage("SOL").showMessage("STAKE")
            .baseConvert(publicKey3, Buffer.CACHE2, 0, ScriptAssembler.base58Charset, ScriptAssembler.zeroInherit)
            .showAddress(ScriptData.getDataBufferAll(Buffer.CACHE2)).clearBuffer(Buffer.CACHE2)
            .baseConvert(createAccountLamports, Buffer.CACHE1, 8, ScriptAssembler.binaryCharset,
                ScriptAssembler.inLittleEndian)
            .showAmount(ScriptData.getDataBufferAll(Buffer.CACHE1), 9).showPressButton().clearBuffer(Buffer.CACHE2)
            .setHeader(HashType.NONE, SignType.EDDSA).getScript();
    }

    public static String getDelegateAndCreateAccountWithSeedScriptSignature = Strings.padStart(
        "3044022026ea0b1c5ab42fc52fa4542db17b9a0a92b42ffc941a53b82ec8cedd7fa4fd1d02200b2edd468c0d6761add53a597774cdc1c5d1599b186a1ade4b2458c559771411",
        144, '0');
    ...
    public static String getUndelegateScript() {
        ...
        return scriptAsb.setCoinType(0x01f5).copyArgument(header).copyArgument(keysCount).copyArgument(publicKey0)
            .copyArgument(publicKey1).copyArgument(publicKey2).copyArgument(publicKey3)
            .ifEqual(publicKey4, EMPTY_PUBLIC_KEY, "", new ScriptAssembler().copyArgument(publicKey4).getScript())
            .copyArgument(recentBlockhash).copyArgument(instructionsCount)
            .ifEqual(gasPriceDataLength, "00", "",
                new ScriptAssembler().copyArgument(gasPriceProgramIdIndex).copyArgument(gasPriceKeyLength)
            ...
            .ifEqual(gasLimitDataLength, "00", "",
                new ScriptAssembler().copyArgument(gasLimitProgramIdIndex).copyArgument(gasLimitKeyLength)
                    .copyArgument(gasLimitDataLength).copyArgument(gasLimitComputeBudgetInstructionType)
                    .copyArgument(gasLimit).getScript())
            .copyArgument(programIdIndex).copyArgument(keyIndicesLength).copyArgument(keyIndices)
            .copyArgument(dataLength).copyArgument(data) // 05000000
            .showMessage("SOL").showMessage("UnDel")
            .baseConvert(publicKey0, Buffer.CACHE2, 0, ScriptAssembler.base58Charset, ScriptAssembler.zeroInherit)
            .showAddress(ScriptData.getDataBufferAll(Buffer.CACHE2)).showPressButton().clearBuffer(Buffer.CACHE2)
            .setHeader(HashType.NONE, SignType.EDDSA).getScript();
    }

    public static String getUndelegateScriptSignature = Strings.padStart(
        "3044022040054884f42bb7257f16c9cc121f15d2466ce8d9efb5e83f7b96973ed29d1f260220328d320b2ce9339123d923e3a2d81ffb6382a3588da19e93d8795d0265dfe561",
        144, '0');
    ...
    public static String getStackingWithdrawScript() {
        ...
        return scriptAsb.setCoinType(0x01f5).copyArgument(header).copyArgument(keysCount).copyArgument(pubkeyKey0)
            .copyArgument(publicKey1).copyArgument(publicKey2).copyArgument(publicKey3).copyArgument(publicKey4)
            .ifEqual(publicKey5, EMPTY_PUBLIC_KEY, "", new ScriptAssembler().copyArgument(publicKey5).getScript())
            ...
            .ifEqual(gasLimitDataLength, "00", "",
                new ScriptAssembler().copyArgument(gasLimitProgramIdIndex).copyArgument(gasLimitKeyLength)
                    .copyArgument(gasLimitDataLength).copyArgument(gasLimitComputeBudgetInstructionType)
                    .copyArgument(gasLimit).getScript())
            .copyArgument(withdrawProgramIdIndex).copyArgument(withdrawKeyIndicesLength).copyArgument(withdrawKeyIndex0)
            .copyArgument(withdrawKeyIndex1).copyArgument(withdrawRemainKeyIndices).copyArgument(withdrawDataLength)
            .copyArgument(withdrawInstruction) // 04000000
            .copyArgument(withdrawData).showMessage("SOL").showMessage("Reward")
            .ifEqual(withdrawKeyIndex1, "00", new ScriptAssembler()
                .baseConvert(pubkeyKey0, Buffer.CACHE2, 0, ScriptAssembler.base58Charset, ScriptAssembler.zeroInherit)
                .showAddress(ScriptData.getDataBufferAll(Buffer.CACHE2)).clearBuffer(Buffer.CACHE2).getScript(), "")
            .ifEqual(withdrawKeyIndex1, "01", new ScriptAssembler()
                .baseConvert(publicKey1, Buffer.CACHE2, 0, ScriptAssembler.base58Charset, ScriptAssembler.zeroInherit)
                .showAddress(ScriptData.getDataBufferAll(Buffer.CACHE2)).clearBuffer(Buffer.CACHE2).getScript(), "")
            .ifEqual(withdrawKeyIndex1, "02", new ScriptAssembler()
                .baseConvert(publicKey2, Buffer.CACHE2, 0, ScriptAssembler.base58Charset, ScriptAssembler.zeroInherit)
                .showAddress(ScriptData.getDataBufferAll(Buffer.CACHE2)).clearBuffer(Buffer.CACHE2).getScript(), "")
            .baseConvert(withdrawData, Buffer.CACHE1, 8, ScriptAssembler.binaryCharset, ScriptAssembler.inLittleEndian)
            .showAmount(ScriptData.getDataBufferAll(Buffer.CACHE1), 9).clearBuffer(Buffer.CACHE1).showPressButton()
            .setHeader(HashType.NONE, SignType.EDDSA).getScript();
    }

    public static String getStackingWithdrawScriptSignature = Strings.padStart(
        "304402200329a67f8ce88f59b5dc7514488c07434fe0dc05cf9e25f151d964fff052eed102204ef7f0b5ae72bf209f093968bbaa48aeda3216103f04667cc6481fd49d82defc",
        144, '0');
}
