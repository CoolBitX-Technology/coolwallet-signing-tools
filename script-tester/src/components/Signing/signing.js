...
import {
  makeSendComponentWithTwoInput,
  makeSendComponentWithThreeInput,
} from './componentMaker';
...
function Signing({ transport, appId, appPrivateKey, isLocked, setIsLocked }) {
  ...
  const signTx = async () => {
    setIsLocked(true);
    try {
      const fullScript = script + scriptSingature;

      const preActions = [];
      const sendScript = async () => {
        await apdu.tx.sendScript(transport, fullScript);
      };
  ...
  const signUtxoTx = async () => {
    ...
    try {
      ...
      const fullScript = utxoScript + scriptSingature;

      console.log('fullScript: ', fullScript);

      await apdu.tx.sendScript(transport, fullScript);

      await executeScript(transport, utxoOutputArgument);
      ...
      const encryptedSignatureArray = await executeUtxoScript(
        transport,
        utxoIntputArgument
      );
  ...
  const executeScript = async (transport, argument) => {
    const {
      ...
      statusCode,
      msg,
    } = await apdu.execute.executeCommand(
      transport,
  ...
  return (
    <Container>
      ...
      {makeSendComponentWithThreeInput(
        ...
        disabled,
        utxoIntputArgument,
        setUtxoIntputArgument,
        'intput arg',
        utxoOutputArgument,
        setUtxoOutputArgument,
        'output arg',
        utxoScript,
        setUtxoScript,
        'script'
      )}
      <TxParser title='General Sign' data={Signature} />
      <TxParser title='UTXO Sign' data={utxoSignature} />
    </Container>
  );
}
